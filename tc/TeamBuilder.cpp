#line 2 "TeamBuilder.cpp"

// Standard includes
#include <bits/stdc++.h>
using namespace std;

#define FOR(i,a,b) for(int i= (int )a ; i < (int )b ; ++i)
#define rep(i,n) FOR(i,0,n)
#define mod 1000000007


class TeamBuilder {
public:
vector <int> specialLocations(vector <string> paths) {
	int n =paths.size();
	int m = paths[0].size();
	vector<vector<int> > dp(n,vector<int>(m,0));
	int in[n];
	int out[n];
	memset(in,0,sizeof in);
	memset(out,0,sizeof out);
	rep(i,n)
		rep(j,n)
			if(paths[i][j]=='1')
				out[i]++,in[j]++,dp[i][j]=1;
	// rep(i,n){		rep(j,n)
	// 	cout<<dp[i][j]<<" ";
	// 	cout<<endl;
	// }
	// cout<< "++++++++++++++++++++\n";
	rep(k,n)
	rep(i,n){
		rep(j,n)
		{		// if(i!=j)
				if(dp[i][k]==1 && dp[k][j]==1 && dp[i][j]!=1)
					dp[i][j]=1,out[i]++,in[j]++;
		}
	}
	// rep(i,n){		rep(j,n)
	// 	cout<<dp[i][j]<<" ";
	// 	cout<<endl;}
	vector<int>ans(2,0);
	rep(i,n){
		bool all=true;
		rep(j,n){
			if(j==i)continue;
			if(dp[i][j]!=1){
				all=false;
				break;
			}
		}
		if(all)
			ans[0]++;
	}
	rep(i,n){
		bool all=true;
		rep(j,n){
			if(j==i)continue;
			if(dp[j][i]!=1)
			{
				all=false;
				break;
			}
		}
		if(all)
			ans[1]++;
	}
	// for(int i=0;i<n;i++){
	// 	if(in[i]>=n-1)
	// 		ans[1]++;
	// 	if(out[i]>=n-1)
	// 		ans[0]++;
	// }
	return ans;
}
};

// BEGIN CUT HERE
#include <ctime>
#include <cmath>
#include <string>
#include <vector>
#include <sstream>
#include <iostream>
#include <algorithm>
using namespace std;

int main(int argc, char* argv[])
{
	if (argc == 1) 
	{
		cout << "Testing TeamBuilder (1000.0 points)" << endl << endl;
		for (int i = 0; i < 20; i++)
		{
			ostringstream s; s << argv[0] << " " << i;
			int exitCode = system(s.str().c_str());
			if (exitCode)
				cout << "#" << i << ": Runtime Error" << endl;
		}
		int T = time(NULL)-1415949023;
		double PT = T/60.0, TT = 75.0;
		cout.setf(ios::fixed,ios::floatfield);
		cout.precision(2);
		cout << endl;
		cout << "Time  : " << T/60 << " minutes " << T%60 << " secs" << endl;
		cout << "Score : " << 1000.0*(.3+(.7*TT*TT)/(10.0*PT*PT+TT*TT)) << " points" << endl;
	}
	else
	{
		int _tc; istringstream(argv[1]) >> _tc;
		TeamBuilder _obj;
		vector <int> _expected, _received;
		time_t _start = clock();
		switch (_tc)
		{
			case 0:
			{
				string paths[] = {"010","000","110"};
				int __expected[] = { 1,  1 };
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}
			case 1:
			{
				string paths[] = {"0010","1000","1100","1000"};
				int __expected[] = { 1,  3 };
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}
			case 2:
			{
				string paths[] = {"01000","00100","00010","00001","10000"};
				int __expected[] = { 5,  5 };
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}
			case 3:
			{
				string paths[] = {"0110000","1000100","0000001","0010000","0110000","1000010","0001000"};
				int __expected[] = { 1,  3 };
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}
			/*case 4:
			{
				string paths[] = ;
				int __expected[] = ;
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}*/
			/*case 5:
			{
				string paths[] = ;
				int __expected[] = ;
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}*/
			/*case 6:
			{
				string paths[] = ;
				int __expected[] = ;
				_expected = vector <int>(__expected, __expected+sizeof(__expected)/sizeof(int));
				_received = _obj.specialLocations(vector <string>(paths, paths+sizeof(paths)/sizeof(string))); break;
			}*/
			default: return 0;
		}
		cout.setf(ios::fixed,ios::floatfield);
		cout.precision(2);
		double _elapsed = (double)(clock()-_start)/CLOCKS_PER_SEC;
		if (_received == _expected)
			cout << "#" << _tc << ": Passed (" << _elapsed << " secs)" << endl;
		else
		{
			cout << "#" << _tc << ": Failed (" << _elapsed << " secs)" << endl;
			cout << "           Expected: {";
			for (unsigned i = 0; i < _expected.size(); i++)
			{
				if (i) cout << ",";
				cout << " " << _expected[i];
			}
			cout << " }" << endl;
			cout << "           Received: {";
			for (unsigned i = 0; i < _received.size(); i++)
			{
				if (i) cout << ",";
				cout << " " << _received[i];
			}
			cout << " }" << endl;
		}
	}
}

// END CUT HERE
